...
public class FTSceneManager : MonoBehaviour
{
    ...
    [SerializeField] private FTSceneData _sceneData;
    ...
    {
        string assetName = "FTSceneData.asset";
        FTSceneData asset = ScriptableObject.CreateInstance<FTSceneData>();
        string sceneDirectory = Path.GetDirectoryName(EditorSceneManager.GetActiveScene().path);

        AssetDatabase.CreateAsset(asset, Path.Combine(sceneDirectory, assetName));
    ...
    public void UpdateFoliage()
    {
        ...
        {
            if (SceneData.FoliageData[i].Matrice.Count > 0)
            {
